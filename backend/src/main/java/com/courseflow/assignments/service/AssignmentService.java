...
public class AssignmentService {
    ...
    /**
     * Get assignment by ID (without courseId in path). Verifies user is enrolled in the course.
     * 
     * @param assignmentId Assignment ID
     * @return Assignment response
     */
    ...
    /**
     * Update an assignment by ID (without courseId in path). Only instructors/admins can update assignments.
     * 
     * @param assignmentId Assignment ID
     * @param request Assignment update request
     * @return Updated assignment response
    ...
    /**
     * Submit an assignment by ID (without courseId in path). Students can submit their work.
     * 
     * @param assignmentId Assignment ID
    ...
    public SubmissionResponse submitAssignment(String courseId, String assignmentId, SubmissionRequest request) {
        ...
        // Check if submission already exists (for SUBMITTED status)
        if (request.getStatus() == Submission.SubmissionStatus.SUBMITTED) {
            Optional<Submission> existingSubmission = submissionRepository.findByAssignmentIdAndStudentId(assignmentId, currentUser.getId());
            if (existingSubmission.isPresent() && existingSubmission.get().getStatus() == Submission.SubmissionStatus.SUBMITTED) {
        ...
        try {
            ...
            submission.setFileUrls(request.getFileUrls() != null ? 
                    new java.util.ArrayList<>(request.getFileUrls()) : new java.util.ArrayList<>());
            
            // Set submittedAt when status changes to SUBMITTED
            if (submission.getStatus() == Submission.SubmissionStatus.SUBMITTED && submission.getSubmittedAt() == null) {
                submission.setSubmittedAt(Instant.now());
            }
            
            submission = submissionRepository.save(submission);
            log.info("Assignment submission {}: assignment {} by student {} in course {}", 
                    submission.getStatus(), assignmentId, currentUser.getId(), courseId);
    ...
    /**
     * Get all submissions for an assignment by ID (without courseId in path). Only instructors can view all submissions.
     * 
     * @param assignmentId Assignment ID
    ...
    /**
     * Grade a submission by ID (without courseId/assignmentId in path). Only instructors/admins can grade submissions.
     * 
    ...
    public SubmissionResponse gradeSubmission(String courseId, String assignmentId, String submissionId, 
        ...
        if (request.getPointsAwarded() != null) {
            ...
            notificationService.notifyUser(
                    ...
                    "Your submission has been graded. Score: " + request.getPointsAwarded() + " / " + assignment.getPoints(),
    ...
    /**
     * Get student's own submission for an assignment by ID (without courseId in path).
     * 
     * @param assignmentId Assignment ID
    ...
    public SubmissionResponse getMySubmissionById(String assignmentId) {
        ...
        Optional<Submission> submission = submissionRepository.findByAssignmentIdAndStudentId(assignmentId, currentUser.getId());
    ...
    private SubmissionResponse mapToSubmissionResponse(Submission submission) {
        ...
        return SubmissionResponse.builder()
                ...
                .fileUrls(submission.getFileUrls() != null ? 
                        new java.util.ArrayList<>(submission.getFileUrls()) : new java.util.ArrayList<>())
